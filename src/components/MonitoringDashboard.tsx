...
const MonitoringDashboard: React.FC<MonitoringDashboardProps> = ({
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      ...
      default: return 'bg-text-slate-500-muted text-white';
    }
  };
  if (isLoading) {
    return (
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center">
            <h1 className="text-3xl font-bold text-slate-900">ConsentHub Monitoring Dashboard</h1>
            <div className="text-sm text-gray-500">
              Last updated: {lastUpdate}
              {showRealTime && <span className="ml-2 inline-block w-2 h-2 bg-green-600 rounded-full"></span>}
            </div>
          </div>
        </div>
                ...
                {healthData?.services.map((service, index) => (
                  <div key={index} className="flex justify-between items-center p-3 border border-gray-200 rounded-lg">
                    <div className="flex items-center space-x-3">
                      <span className="font-medium text-slate-900 capitalize">{service.service}</span>
                ...
                {activeAlerts.map((alert, index) => (
                  <div key={index} className="flex items-center justify-between p-4 border border-red-200 rounded-lg bg-red-50">
                    ...
                    <div className="text-right">
                      ...
                      {!alert.acknowledged && (
                        <button className="mt-1 px-3 py-1 bg-white text-white text-xs rounded hover:bg-slate-900">
                          Acknowledge
                        </button>
                      )}
                    </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th scope="col" className="text-left py-2">Endpoint</th>
                    <th scope="col" className="text-right py-2">Avg Response Time</th>
                    <th scope="col" className="text-right py-2">Request Count</th>
                  </tr>
                </thead>
                <tbody>
