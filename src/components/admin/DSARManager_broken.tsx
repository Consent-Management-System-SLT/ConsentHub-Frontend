...
interface DSARRequest {
  ...
  customerName: string;
  email: string;
  requestType: 'data_access' | 'data_portability' | 'data_erasure' | 'data_rectification' | 'restrict_processing';
...
const DSARManager: React.FC = () => {
  ...
  // Load DSAR requests from backend
  useEffect(() => {
    loadDSARRequests();
  }, []);
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const transformedRequests = response.data.requests.map(req => ({
          ...
          customerId: req.requesterId,
          customerName: req.requesterName,
          email: req.requesterEmail,
          requestType: req.requestType === 'data_access' ? 'access' as const : 
                      req.requestType === 'data_erasure' ? 'deletion' as const :
                      req.requestType === 'data_portability' ? 'portability' as const :
                      req.requestType === 'data_rectification' ? 'rectification' as const :
                      'restriction' as const,
          status: req.status === 'in_progress' ? 'in-progress' as const : req.status as any,
          priority: req.priority,
          requestDate: req.submittedAt.split('T')[0],
          dueDate: req.dueDate.split('T')[0],
          assignedTo: req.assignedTo?.name,
          description: req.description,
          attachments: 0, // Default to 0, can be enhanced later
        ...
        const newStats = {
          ...
          urgent: transformedRequests.filter(req => req.priority === 'urgent').length
        };
        setStats(newStats);
        
      } else {
        throw new Error(response.error || 'Failed to load DSAR requests');
      }
    } catch (err) {
      console.error('Error loading DSAR requests:', err);
      setError(err instanceof Error ? err.message : 'Failed to load DSAR requests');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-full mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-8 w-full box-border">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            ...
            <div className="hidden sm:flex items-center space-x-3">
              ...
              <button 
                ...
                disabled={loading}
                className="px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center space-x-2"
              >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 lg:gap-6 mb-6 sm:mb-8 w-full overflow-hidden">
          <div className="bg-myslt-card-solid rounded-lg sm:rounded-xl shadow-sm border border-myslt-border p-3 sm:p-4 lg:p-6 min-w-0">
            ...
            <div className="bg-white rounded-lg border border-myslt-border p-3 sm:p-4 flex items-center justify-between">
              <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-lg sm:text-xl font-bold text-myslt-text-primary">
                    {stats.in_progress}
                  </p>
                </div>
              </div>
