...
const TopicPreferences: React.FC = () => {
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          <div className="w-8 h-8 border-4 border-slate-200 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-slate-600">Loading topic preferences...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-full overflow-x-hidden">
      <div className="mb-6">
        <h2 className="text-2xl font-semibold text-slate-900">Topic Preferences</h2>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6">
        <div className="space-y-8">
          {/* Topic Preferences */}
          <div>
            <h3 className="text-lg font-medium text-slate-900 mb-4">Communication Topics</h3>
            {Object.entries(groupedTopics).map(([category, categoryTopics]) => (
              <div key={category} className="mb-8">
                <h4 className="text-md font-medium text-slate-900 mb-4 px-3 py-2 bg-slate-100 rounded-lg">
                  {category}
                </h4>
                <div className="space-y-4">
                  {categoryTopics.map(topic => {
                    ...
                    return (
                      <div key={topic.id} className="border border-slate-200 rounded-lg p-4 bg-slate-100">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center">
                            <label className="flex items-center">
                              <input
                                ...
                                className="mr-3 h-4 w-4 text-blue-600 focus:ring-blue-600 border-slate-200 rounded bg-white border-slate-300 focus:ring-blue-500 focus:ring-2"
                              />
                          ...
                          <select
                            ...
                            className="bg-white border border-slate-300 rounded-lg px-3 py-2 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors py-1 text-sm disabled:bg-slate-50/20 disabled:text-slate-500"
                          >
                        ...
                        {preference.enabled && (
                          <div className="ml-7">
                            <p className="text-sm text-slate-600 mb-3">Communication Channels:</p>
                              ...
                              {channels.map(channel => (
                                <label key={channel.id} className="flex items-center">
                                  <input
                                    ...
                                    className="mr-2 h-4 w-4 text-blue-600 focus:ring-blue-600 border-slate-200 rounded bg-white border-slate-300 focus:ring-blue-500 focus:ring-2"
                                  />
                                  <span className="text-sm text-slate-900 flex items-center">
                                    <channel.icon className="w-4 h-4 text-slate-500 mr-1" />
                                    {channel.name}
                                  </span>
                                </label>
                              ))}
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-slate-900">Do Not Disturb Periods</h3>
              <button
                onClick={addDoNotDisturbPeriod}
                className="bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg px-4 py-2 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 text-sm"
              >
                Add Period
              </button>
            </div>
            <div className="space-y-4">
              {doNotDisturbPeriods.map((period, index) => (
                <div key={index} className="border border-slate-200 rounded-lg p-4 bg-slate-100">
                  <div className="flex items-center justify-between mb-4">
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                      ...
                      <input
                        ...
                        className="font-medium text-slate-900 bg-transparent border-none focus:outline-none focus:bg-slate-100 px-2 py-1 rounded"
                      />
                    </label>
                    <button
                      ...
                      <div className="flex items-center space-x-4">
                        <div>
                          ...
                          <input
                            ...
                            className="bg-white border border-slate-300 rounded-lg px-3 py-2 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors mt-1 p-2 text-sm"
                          ...
                          {weekDays.map(day => (
                            <label key={day.id} className="flex items-center">
                              <input
                                ...
                                className="mr-1 h-4 w-4 text-blue-600 focus:ring-blue-600 border-slate-200 rounded bg-white border-slate-300 focus:ring-blue-500 focus:ring-2"
                              />
          ...
          <div className="flex justify-end pt-6 border-t border-slate-200">
            <button
              onClick={handleSave}
              disabled={saving}
              className="bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg px-4 py-2 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 px-6 disabled:opacity-50 disabled:cursor-not-allowed"
            >
