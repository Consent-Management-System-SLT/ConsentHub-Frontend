        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 mb-8">
            {webhooks.length > 0 ? webhooks.map((webhook) => (
              <div key={webhook._id} className="bg-slate-100 border-2 border-slate-200 p-6 hover:shadow-lg hover:border-slate-200 transition-all duration-200 rounded-xl">
...
const LogsModal: React.FC<LogsModalProps> = ({ webhook, logs, onClose }) => {
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default:
        return 'text-slate-500 border border-slate-200';
    }
  };
  const formatDate = (dateString: string) => {
    return new Date(dateString).toLocaleString();
