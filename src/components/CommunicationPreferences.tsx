...
import { Settings, Mail, MessageSquare, Bell, Phone, Edit2, Save, X } from 'lucide-react';
...
export const CommunicationPreferences: React.FC<CommunicationPreferencesProps> = ({ selectedCustomer }) => {
  ...
  const filteredPreferences = selectedCustomer 
    ? preferences.filter(pref => pref.partyId === selectedCustomer.id)
    : preferences;
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <input
                      ...
                      className="px-3 py-1 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-50"
                    />
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <input
                        ...
                        max="20"
                        value={editingId === preference.id 
                          ? editForm.frequencyLimits?.email ?? preference.frequencyLimits.email
                          : preference.frequencyLimits.email
                        }
                        onChange={(e) => {
                          if (editingId === preference.id) {
                            setEditForm(prev => ({
                        ...
                        disabled={editingId !== preference.id}
                        className="w-16 px-2 py-1 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-50"
                      />
                    </div>
                    <div className="flex items-center space-x-2">
                      ...
                      <input
                        ...
                        max="10"
                        value={editingId === preference.id 
                          ? editForm.frequencyLimits?.sms ?? preference.frequencyLimits.sms
                          : preference.frequencyLimits.sms
                        }
                        onChange={(e) => {
                          if (editingId === preference.id) {
                            setEditForm(prev => ({
