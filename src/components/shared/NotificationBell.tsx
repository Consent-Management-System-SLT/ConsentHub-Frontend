  ...
  return (
    <div className={`relative ${className}`}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className={`absolute -top-1 -right-1 ${sizeClasses.badge} rounded-full bg-red-600 ring-2 ring-myslt-primary font-bold text-white flex items-center justify-center`}>
      ...
      {isOpen && (
        <div 
          ref={dropdownRef}
          className="bg-white border border-slate-200 shadow-lg absolute right-0 top-12 w-96 rounded-xl shadow-2xl z-50 max-h-[80vh] flex flex-col"
        >
          {/* Header */}
          <div className="flex items-center justify-between p-4 border-b border-slate-200">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                className="p-1 hover:bg-blue-50/20 rounded-full transition-colors"
              >
                <X className="w-4 h-4 text-slate-500 text-slate-500" />
              </button>
            </div>
          </div>
            ...
            {displayedNotifications.length === 0 ? (
              ...
              <div className="divide-y divide-myslt-accent/30">
                {displayedNotifications.map((notification) => (
                  <div
                    key={notification.id}
                    className={`p-4 hover:bg-blue-50/20 transition-colors cursor-pointer group ${
                      !notification.read ? 'bg-blue-50/10 border-l-4 border-l-myslt-primary' : ''
                    }`}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between">
                          ...
                          <button
                            ...
                            className="flex-shrink-0 p-1 hover:bg-blue-50/30 rounded-full transition-colors opacity-0 group-hover:opacity-100"
                          >
          ...
          {notifications.length > maxDisplayed && (
            <div className="p-3 border-t border-gray-100 text-center">
              <button className="text-sm text-blue-600 hover:text-slate-900 font-medium">
