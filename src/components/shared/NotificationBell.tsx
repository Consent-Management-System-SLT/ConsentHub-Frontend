  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {isOpen && (
        <div 
          ref={dropdownRef}
          className="bg-white border border-slate-200 shadow-lg absolute right-0 top-12 w-96 rounded-xl z-50 max-h-[80vh] flex flex-col"
        >
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto">
            {displayedNotifications.length === 0 ? (
              ...
              <div className="divide-y divide-blue-600/30">
                {displayedNotifications.map((notification) => (
                  <div
                    key={notification.id}
                    className={`p-4 hover:bg-blue-50/20 transition-colors cursor-pointer group ${
                      !notification.read ? 'bg-blue-50/10 border-l-4 border-l-blue-600' : ''
                    }`}
