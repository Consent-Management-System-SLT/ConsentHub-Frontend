...
export const DSARRequests: React.FC<DSARRequestsProps> = ({ selectedCustomer }) => {
  ...
  const DSARModal = () => {
    ...
    return (
      <div role="dialog" aria-modal="true" aria-label="DSAR Request Details" className="fixed inset-0 bg-slate-900/50 flex items-center justify-center p-4 z-50">
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <thead className="bg-gray-50">
              <tr>
                {['Customer', 'Type', 'Status', 'Submitted', 'Completed', 'Actions'].map((h) => (
                  <th scope="col" key={h} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{h}</th>
                ))}
              </tr>
            </thead>
