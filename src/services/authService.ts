...
export interface User {
  ...
  department?: string;
  jobTitle?: string;
  role: 'customer' | 'csr' | 'admin' | 'enterprise' | 'guardian';
  partyId?: string;
  ...
  async login(credentials: LoginRequest): Promise<AuthResponse> {
    try {
      ...
      try {
        ...
        // No client-side fallback: authentication is decided by the backend only.
        throw new Error(
          backendError?.message || 'Unable to reach the authentication service. Please try again.'
        );
      }
