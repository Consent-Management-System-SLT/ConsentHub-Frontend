...
export class MultiServiceApiClient {
  ...
  async get(endpoint: string, options: { role?: string; service?: string } = {}): Promise<{ data: any }> {
    return { data: await this.makeRequest('GET', endpoint, undefined, options.role, options.service) };
  }
  async post(endpoint: string, data?: any, options: { role?: string; service?: string } = {}): Promise<{ data: any }> {
    return { data: await this.makeRequest('POST', endpoint, data, options.role, options.service) };
  }
  async put(endpoint: string, data?: any, options: { role?: string; service?: string } = {}): Promise<{ data: any }> {
    return { data: await this.makeRequest('PUT', endpoint, data, options.role, options.service) };
  }
  async delete(endpoint: string, options: { role?: string; service?: string } = {}): Promise<{ data: any }> {
    return { data: await this.makeRequest('DELETE', endpoint, undefined, options.role, options.service) };
  }
